import pandas as pd
from fair.data.objects.loan_defaults_obj import LoanDefaultsData
...
df_obj = LoanDefaultsData()

results = pd.DataFrame(columns = ["algorithm", "run_id", "accuracy","TPR", "TNR", "FPR", "FNR"])
results_by_group = pd.DataFrame(columns = ["algorithm", "run_id", "sex", "Count", "Selection_rate", "Accuracy", "TPR", "TNR", "FPR", "FNR"])    
...
dp_diff_baseline = demographic_parity_difference(df_obj.y, df_obj.y, sensitive_features = df_obj.sensitive_attributes)
dp_ratio_baseline  = demographic_parity_ratio(df_obj.y, df_obj.y, sensitive_features = df_obj.sensitive_attributes)
...
baseline.to_csv('experiments/results/Experiment 1/loans-baseline.csv')

for i in range(10):
    
    X_train, X_test, y_train, y_test, S_train, S_test, S_train_num, S_test_num = df_obj.train_test_split(train_size = 0.7)
    ...
    mssg_obj = Massaging(X_train, y_train, S_train, sens_group_name = df_obj.sens_group_name, 
                          non_sens_group_name = df_obj.non_sens_group_name)
    mssg_obj.fit()
    ...
    samp_obj = PreferentialSampling(X_train, y_train, S_train, 
                                    sens_group_name = df_obj.sens_group_name, 
                                    non_sens_group_name = df_obj.non_sens_group_name)
    samp_obj.fit()
    ...
    rewei_obj = Reweighing(X_train, y_train, S_train, sens_group_name = df_obj.sens_group_name,
                            non_sens_group_name = df_obj.non_sens_group_name)
    rewei_obj.fit()
    ...
    w = disp_imp_score_obj.fit(dataset = df_obj, clstype = 'log-reg', sensitive = df_obj.sens_attr_names)
    ...
    w_svm = disp_imp_score_obj_svm.fit(dataset = df_obj, clstype = 'lin-svm', sensitive = df_obj.sens_attr_names)
    y_pred_zafar_svm = disp_imp_score_obj_svm.predict(X_test)
    print("Disparate Impact Score SVM on", df_obj.data_name, "done.")
    
    estimator_exp = LogisticRegression(fit_intercept = True)
    estimator_exp.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_exp_2 = LogisticRegression(fit_intercept = True)
    estimator_exp_2.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_grid = LogisticRegression(fit_intercept = True)
    estimator_grid.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_grid_2 = LogisticRegression(fit_intercept = True)
    estimator_grid_2.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_eq = LogisticRegression(fit_intercept = True)
    estimator_eq.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_eq_2 = LogisticRegression(fit_intercept = True)
    estimator_eq_2.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    exp_gradient_obj = Exponentiated_Gradient(df_obj.sens_attr_names, constraints = "demographic parity", estimator = estimator_exp)
    ...
    grid_search_obj = Grid_Search(df_obj.sens_attr_names, constraints = "demographic parity", estimator = estimator_grid)
    ...
    exp_gradient_obj_2 = Exponentiated_Gradient(df_obj.sens_attr_names, constraints = "equalized odds", estimator = estimator_exp_2)
    ...
    grid_search_obj_2 = Grid_Search(df_obj.sens_attr_names, constraints = "equalized odds", estimator = estimator_grid_2)
    ...
    eq_of_oppor_obj = EqualityOfOpportunity(estimator = estimator_eq)
    ...
    eq_of_oppor_obj_2 = EqualityOfOpportunity(estimator = estimator_eq_2, constraints='equalized_odds')
    ...
    for l in [y_pred_unmitigated, y_pred_mssg, y_pred_samp, y_pred_rewei, y_pred_corr,
              ...
              y_pred_exp_gradient_2, y_pred_zafar, y_pred_zafar_svm, y_pred_eq_opp, y_pred_eq_op_2]:
        ...
        for p in [df_obj.sens_group_name, df_obj.non_sens_group_name]:
            results_by_group = results_by_group.append({'algorithm':algo_names[n], 'run_id': i,
                                                        'sex': p, 'Count': metric_frame.by_group["Count"][k],
...
for y in algo_names:
    ...
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["sex"] == df_obj.sens_group_name)].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["sex"] == df_obj.non_sens_group_name)].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
...
df_race = results_by_group["sex"].reset_index(drop=True).head(20)
mean_results_by_group = mean_results_by_group.join([df_race, df_means_by_group])

results_overall.to_csv('experiments/results/Experiment 3/In_in/loans-di_score_in_post-overall.csv')
results_by_group.to_csv('experiments/results/Experiment 3/In_in/loans-di_score_in_post-by_group.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 3/In_in/loans-di_score_in_post-mean_by_group.csv')
mean_results_overall.to_csv('experiments/results/Experiment 3/In_in/loans-di_score_in_post-mean_overall.csv')
