...
algo_names = ["Unmitigated", "Massaging", "Pref. Sampling", "Reweighing", "Corr. Remover",
              "DI Remover", "Grid Search", "Exp. Gradient", "DI Score", "Equal. Of Oppor."]
algo_names_dup = ["Unmitigated", "Unmitigated", "Massaging", "Massaging", "Pref. Sampling", "Pref. Sampling", "Reweighing","Reweighing", "Corr. Remover","Corr. Remover",
              "DI Remover","DI Remover", "Grid Search","Grid Search", "Exp. Gradient", "Exp. Gradient", "DI Score", "DI Score", "Equal. Of Oppor.", "Equal. Of Oppor."]
...
for i in range(10):
    ...
    X_train, X_test, y_train, y_test, S_train, S_test, S_train_num, S_test_num = df_obj.train_test_split(train_size = 0.7, sensitive = 'sex')
    ...
    #sensitive_group_name[0] for sex and sensitive_group_name[1] for race
    ...
    corr_predictor = LogisticRegression(solver = 'liblinear',fit_intercept = True)
    ...
    w = disp_imp_score_obj.fit(dataset = df_obj, clstype = 'log-reg')
    ...
    exp_gradient_obj = Exponentiated_Gradient(df_obj.sens_attr_names[0], estimator = estimator_exp)
    exp_gradient_obj.fit(X_train, y_train, S_train)
    y_pred_exp_gradient = exp_gradient_obj.predict(X_test)
    print("Exponentiated Gradient on", df_obj.data_name, "done.")
    results_exp = pd.DataFrame({"Exp. Gradient": y_pred_exp_gradient})
    
    grid_search_obj = Grid_Search(df_obj.sens_attr_names[0], estimator = estimator_grid)
    grid_search_obj.fit(X_train, y_train, S_train)
    y_pred_grid = grid_search_obj.predict(X_test)
    print("Grid Search on", df_obj.data_name, "done.")
    ...
    print("Equality of Opportunity on", df_obj.data_name, "done.")
    ...
    for l in [y_pred_unmitigated, y_pred_mssg, y_pred_samp, y_pred_rewei, y_pred_corr,
              y_pred_dispRemover, y_pred_grid, y_pred_exp_gradient, y_pred_zafar, 
              y_pred_eq_opp]:
...
results_overall.to_csv('experiments/results/Experiment 2/compas_sex-in_in-overall.csv')
results_by_group.to_csv('experiments/results/Experiment 2/compas_sex-in_in-by_group.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 2/compas_sex-in_in-mean_by_group.csv')
mean_results_overall.to_csv('experiments/results/Experiment 2/compas_sex-in_in-mean_overall.csv')
