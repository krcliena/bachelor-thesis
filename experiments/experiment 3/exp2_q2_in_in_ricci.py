...
from fair.data.objects.ricci_obj import RicciData
...
#from fair.algorithms.inprocessing.disparate_mistreatment_score import DisparateMistreatmentScore
...
baseline.to_csv('experiments/results/Experiment 1/ricci-baseline.csv')

for i in range(10):
    X_train, X_test, y_train, y_test, S_train, S_test, S_train_num, S_test_num = df_obj.train_test_split(train_size = 0.7)
    
    unmitigated_predictor = LogisticRegression(fit_intercept =True)
    ...
    metric_frame_rewei = MetricFrame(metric = {"Accuracy": skm.accuracy_score,
...
for y in algo_names:
    ...
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["Race"] == df_obj.sens_group_name)].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["Race"] == df_obj.non_sens_group_name)].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
...
results_overall.to_csv('experiments/results/Experiment 3/In_in/ricci-di_score_in_post-overall.csv')
results_by_group.to_csv('experiments/results/Experiment 3/In_in/ricci-di_score_in_post-by_group.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 3/In_in/ricci-di_score_in_post-mean_by_group.csv')
mean_results_overall.to_csv('experiments/results/Experiment 3/In_in/ricci-di_score_in_post-mean_overall.csv')
