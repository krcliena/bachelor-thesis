...
from fair.data.objects.german_obj import GermanData
...
for i in range(10):
    ...
    X_train, X_test, y_train, y_test, S_train, S_test, S_train_num, S_test_num, _, _ = df_obj.train_test_split(train_size = 0.7, sensitive = 'sex')
    ...
    mssg_obj = Massaging(X_train, y_train, S_train, sens_group_name = df_obj.sens_group_name[0], 
                          non_sens_group_name = df_obj.non_sens_group_name[0])
    mssg_obj.fit()
    ...
    samp_obj = PreferentialSampling(X_train, y_train, S_train, 
                                    sens_group_name = df_obj.sens_group_name[0], 
                                    non_sens_group_name = df_obj.non_sens_group_name[0])
    samp_obj.fit()
    ...
    rewei_obj = Reweighing(X_train, y_train, S_train, sens_group_name = df_obj.sens_group_name[0],
                            non_sens_group_name = df_obj.non_sens_group_name[0])
    rewei_obj.fit()
    ...
    #sens_attr_names[1] = age, sens_attr_names[0] = sex
    ...
    w = disp_imp_score_obj.fit(dataset = df_obj, clstype = 'log-reg', sensitive = df_obj.sens_attr_names[0])
    ...
    w_svm = disp_imp_score_obj_svm.fit(dataset = df_obj, clstype = 'lin-svm', sensitive = df_obj.sens_attr_names[0])
    ...
    exp_gradient_obj = Exponentiated_Gradient(df_obj.sens_attr_names[0], constraints = "demographic parity", estimator = estimator_exp)
    ...
    grid_search_obj = Grid_Search(df_obj.sens_attr_names[0], constraints = "demographic parity", estimator = estimator_grid)
    ...
    exp_gradient_obj_2 = Exponentiated_Gradient(df_obj.sens_attr_names[0], constraints = "equalized odds", estimator = estimator_exp_2)
    ...
    grid_search_obj_2 = Grid_Search(df_obj.sens_attr_names[0], constraints = "equalized odds", estimator = estimator_grid_2)
    ...
    for l in [y_pred_unmitigated, y_pred_mssg, y_pred_samp, y_pred_rewei, y_pred_corr,
        ...
        for p in [df_obj.sens_group_name[0], df_obj.non_sens_group_name[0]]:
...
for y in algo_names:
    ...
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["sex"] == df_obj.sens_group_name[0])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["sex"] == df_obj.non_sens_group_name[0])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
...
results_overall.to_csv('experiments/results/Experiment 3/In_in/german_sex-di_score_in_post-overall.csv')
results_by_group.to_csv('experiments/results/Experiment 3/In_in/german_sex-di_score_in_post-by_group.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 3/In_in/german_sex-di_score_in_post-mean_by_group.csv')
mean_results_overall.to_csv('experiments/results/Experiment 3/In_in/german_sex-di_score_in_post-mean_overall.csv')
