...
results_by_group = pd.DataFrame(columns = ["algorithm", "run_id", "age", "Count", "Selection_rate", "Accuracy", "TPR", "TNR", "FPR", "FNR"])    
...
for i in range(10):
    ...
    #sensitive = 'sex' if we want the sensitive attribute to be sex
    X_train, X_test, y_train, y_test, S_train, S_test, S_train_num, S_test_num, _,_ = df_obj.train_test_split(train_size = 0.7)
    ...
    exp_gradient_obj = Exponentiated_Gradient(df_obj.sens_attr_names[1], constraints = "demographic parity", estimator = estimator_exp)
    ...
    grid_search_obj = Grid_Search(df_obj.sens_attr_names[1], constraints = "demographic parity", estimator = estimator_grid)
    ...
    exp_gradient_obj_2 = Exponentiated_Gradient(df_obj.sens_attr_names[1], constraints = "equalized odds", estimator = estimator_exp_2)
    ...
    grid_search_obj_2 = Grid_Search(df_obj.sens_attr_names[1], constraints = "equalized odds", estimator = estimator_grid_2)
    ...
    for l in [y_pred_unmitigated, y_pred_mssg, y_pred_samp, y_pred_rewei, y_pred_corr,
        ...
        results = results.append({'algorithm': algo_names[n], 'run_id': i, 
                                         ...
                                         ignore_index=True)
        k = 0
        for p in [df_obj.non_sens_group_name[1], df_obj.sens_group_name[1]]:
            results_by_group = results_by_group.append({'algorithm':algo_names[n], 'run_id': i,
                                                        'age': p, 'Count': metric_frame.by_group["Count"][k],
...
for y in algo_names:
    ...
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["age"] == df_obj.non_sens_group_name[1])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["age"] == df_obj.sens_group_name[1])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
...
df_race = results_by_group["age"].reset_index(drop=True).head(20)
mean_results_by_group = mean_results_by_group.join([df_race, df_means_by_group])

results_overall.to_csv('experiments/results/Experiment 3/In_in/german_age-di_score_in_post-overall.csv')
results_by_group.to_csv('experiments/results/Experiment 3/In_in/german_age-di_score_in_post-by_group.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 3/In_in/german_age-di_score_in_post-mean_by_group.csv')
mean_results_overall.to_csv('experiments/results/Experiment 3/In_in/german_age-di_score_in_post-mean_overall.csv')
