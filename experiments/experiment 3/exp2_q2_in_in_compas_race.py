...
dp_diff_baseline = demographic_parity_difference(df_obj.y, df_obj.y, sensitive_features = df_obj.sensitive_attributes["race"])
dp_ratio_baseline = demographic_parity_ratio(df_obj.y, df_obj.y, sensitive_features = df_obj.sensitive_attributes["race"])
...
baseline.to_csv('experiments/results/Experiment 1/compas_race-baseline.csv')
...
for i in range(10):
    ...
    estimator_exp = LogisticRegression(solver = 'liblinear',fit_intercept = True)
    estimator_exp.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_exp_2 = LogisticRegression(solver = 'liblinear',fit_intercept = True)
    estimator_exp_2.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_grid = LogisticRegression(solver = 'liblinear',fit_intercept = True)
    estimator_grid.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_grid_2 = LogisticRegression(solver = 'liblinear',fit_intercept = True)
    estimator_grid_2.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_eq = LogisticRegression(solver = 'liblinear',fit_intercept = True)
    estimator_eq.coef_ = np.reshape(w.w, (1,len(w.w)))
    
    estimator_eq_2 = LogisticRegression(solver = 'liblinear',fit_intercept = True)
...
results_overall.to_csv('experiments/results/Experiment 3/In_in/compas_race-di_score_in_post-overall.csv')
results_by_group.to_csv('experiments/results/Experiment 3/In_in/compas_race-di_score_in_post-by_group.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 3/In_in/compas_race-di_score_in_post-mean_by_group.csv')
mean_results_overall.to_csv('experiments/results/Experiment 3/In_in/compas_race-di_score_in_post-mean_overall.csv')
