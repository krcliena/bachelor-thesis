...
#Optimize for sex assess on age
...
algo_names = ["Unmitigated","DI score"]
...
algo_names_dup = ["Unmitigated","Unmitigated","DI score", "DI score"]
...
for i in range(10):
    ...
    # exp_gradient_obj = Exponentiated_Gradient(df_obj.sens_attr_names[1])
    ...
    # grid_search_obj = Grid_Search(df_obj.sens_attr_names[1])
    ...
    w = disp_imp_score_obj.fit(dataset = df_obj, clstype = 'lin-svm', sensitive =df_obj.sens_attr_names[0])
    ...
    #Evaluation + Results
    n = 0
    for l in [y_pred_unmitigated, y_pred_zafar]:
...
for y in algo_names:
    ...
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["age"] == df_obj.sens_group_name[0])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["age"] == df_obj.non_sens_group_name[0])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
...
#results_overall.to_csv('experiments/results/Experiment 2/german_age-LR-results_overall-exp2.csv')
#results_by_group.to_csv('experiments/results/Experiment 2/german_age-LR-results_by_group-exp2.csv')
mean_results_by_group.to_csv('experiments/results/Experiment 2/german_sex-zafar-exp2.csv')
mean_results_overall.to_csv('experiments/results/Experiment 2/german_sex-zafar-exp2.csv')
