...
for i in range(10):
    X_train, X_test, y_train, y_test, S_train, S_test, S_train_num, S_test_num,_, S_test_oth = df_obj.train_test_split(train_size = 0.7)
...
for y in algo_names:
    ...
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["race"] == df_obj.sens_group_name[0])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
    means_by_group = results_by_group[(results_by_group["algorithm"] == y) & (results_by_group["race"] == df_obj.non_sens_group_name[0])].drop(labels='run_id', axis=1).mean().to_frame().T
    df_means_by_group = df_means_by_group.append(means_by_group)
...
mean_results_by_group.to_csv('experiments/results/Experiment 2/adult_sex-zafar-exp2.csv')
mean_results_overall.to_csv('experiments/results/Experiment 2/adult_sex-zafar-exp2.csv')
