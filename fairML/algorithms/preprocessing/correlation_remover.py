...
class CorrelationRemover(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        self.X_use = X_use
        self.test = X_s_center.dot(self.beta_)
        X_use = np.atleast_2d(X_use)
        self.X_s_center = X_s_center
        X_filtered = np.atleast_2d(X_filtered)
        #Important to return X_sensitive?
        X_sensitive = pd.DataFrame(X_sensitive, columns = self.sensitive_feature_ids)
        return self.alpha * X_filtered + (1 - self.alpha) * X_use, X_sensitive