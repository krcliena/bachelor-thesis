...
import fair.algorithms.inprocessing.fair_classification.loss_funcs as lf # our implementation of loss funcs
...
def compute_single_fold(X, y, x_control, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs,sensitive_attrs_to_cov_thresh, gamma=None):

    X_train, X_test = X['train'], X['test']
    y_train, y_test = y['train'], y['test']
    x_control_train, x_control_test = x_control['train'], x_control['test']

    print("Here", sensitive_attrs)
    w = train_model(X_train, y_train, x_control_train, loss_function, apply_fairness_constraints, apply_accuracy_constraint,
                sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh, gamma)

    train_score, test_score, correct_answers_train, correct_answers_test = check_accuracy(w, X_train, y_train, X_test,
                                                                                          y_test, None, None)
    ...
    return test_score, train_score, correlation_dict_test, correlation_dict_train, cov_dict_test, cov_dict_train, w
...
def compute_cross_validation_error(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh_arr, gamma=None):
    ...
    """
    #print(sensitive_attrs_to_cov_thresh_arr)
    train_folds = []
    test_folds = []
    n_samples = len(y_all)
...
def print_classifier_fairness_stats_single_fold(acc_, correlation_dict, s_attr_name):
...
def compute_p_rule(x_control, class_labels):
    ...
    x_control = list(map(int, x_control))
    class_labels = list(map(int, class_labels))
    #non_prot_all = sum(x_control == 1) # non-protected group
    non_prot_all = sum([1 for i in x_control if i == 1])
    #prot_all = sum(x_control == 0) # protected group
    prot_all = sum([1 for i in x_control if i == 0])
    #non_prot_pos = sum(class_labels[x_control == 1] == 1) # non_protected in positive class
    non_prot_pos = sum([1 for i in range(len(x_control)) if x_control[i] == 1 and class_labels[i] == 1])
    #prot_pos = sum(class_labels[x_control == 0] == 1) # protected in positive class
    prot_pos = sum([1 for i in range(len(x_control)) if x_control[i] == 0 and class_labels[i] == 1])
...
def get_predictions(model, X_test, y_test):
    ...
    for i in range(len(y_test_predicted)):
       if y_test_predicted[i]==y_test[i]:
           print(f"correct -- {y_test_predicted[i]}, {y_test[i]}")
           correct_answers+=1
       else:
           print(f"correct -- {y_test_predicted[i]}, {y_test[i]}")

    print(correct_answers/len(y_test))
...
def get_constraint_list_cov(x_train, y_train, x_control_train, sensitive_attrs, sensitive_attrs_to_cov_thresh):
    ...
    constraints = []
    for attr in sensitive_attrs:
        attr_arr = list(map(int, x_control_train[attr]))
        #print(f"attr_arr - {attr_arr}")
        attr_arr_transformed, index_dict = get_one_hot_encoding(attr_arr)
