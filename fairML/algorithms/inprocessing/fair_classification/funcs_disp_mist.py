...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    if loss_function == "logreg":
        ...
        loss = cvx.sum(logistic( cvx.multiply(-y, x*w) ) ) / num_points # we are converting y to a diagonal matrix for consistent

    elif loss_function == "linear_svm":
        loss = cvx.sum(cvx.pos(1 - cvx.multiply(y, x@w - b))) + cvx.norm(w, 2)
    ...
    for attr in sensitive_attrs_to_cov_thresh.keys():
        ...
        if index_dict is None: # binary attribute, in this case, the attr_arr_transformed is the same as the attr_arr
            ...
            for v in set(attr_arr):
                ...
                s_val_to_total[0][v] = sum([1 for x in x_control_train[attr] if x == v])
                ...
                s_val_to_total[1][v] = sum([1 for i in range(len(x_control_train[attr]))
                                            if x_control_train[attr][i] == v and y_train[i] == -1])
                #s_val_to_total[2][v] = sum(np.logical_and(x_control_train[attr] == v, y_train == +1))
                s_val_to_total[2][v] = sum([1 for i in range(len(x_control_train[attr]))
                                            if x_control_train[attr][i] == v and y_train[i] == 1])
...
def get_fpr_fnr_sensitive_features(y_true, y_pred, x_control, sensitive_attrs, verbose = False):
    ...
    for s in sensitive_attrs:
        ...
        for s_val in sorted(list(set(s_attr_vals))):
            ...
            acc = sum([1 for i in range(len(y_true_local)) if y_pred_local[i]==y_true_local[i]])/len(y_true_local)
            ...
            tn, fp, fn, tp = confusion_matrix(y_true_local, y_pred_local).ravel()
...
def get_sensitive_attr_constraint_fpr_fnr_cov(model, x_arr, y_arr_true, y_arr_dist_boundary, x_control_arr, verbose=False):
    ...
    for v in set(x_control_arr):
        ...
        s_val_to_total[0][v] = sum([1 for x in x_control_arr if x == v])
        #s_val_to_total[1][v] = sum(np.logical_and(x_control_arr == v, y_arr_true == -1))
        s_val_to_total[1][v] = sum([1 for i in range(len(x_control_arr)) if x_control_arr[i] == v
                                    and y_arr_true[i] == -1])
        #s_val_to_total[2][v] = sum(np.logical_and(x_control_arr == v, y_arr_true == +1))
        s_val_to_total[2][v] = sum([1 for i in range(len(x_control_arr)) if x_control_arr[i] == v
                                    and y_arr_true[i] == 1])
